...
//    }
    ...
    public ArrayList<Long> getParseData(ParseUser user , final String pointer, final String ... params) //fetch from Parse
    ...
    public void pushParseDataSimpleTap(ParseUser user, String ... params) //upload to Parse single row
    //0: class name -- 1: column name -- 2: json String
    // user: pointer to ParseUser (use ParseUser.getCurrentUser())
    //you may add additional String parameters as necessary
    {
    ...
    public ArrayList<AltTapData> getParseDataAltTap(ParseUser user , final String position, final String ... params) //fetch from Parse
    //0: class name -- 1: orderby String  -- 2: column name
    // user: pointer to ParseUser (use ParseUser.getCurrentUser())
    // you may add additional String parameters as necessary
    {
        ParseQuery<ParseObject> query = ParseQuery.getQuery(params[0]);
        query.whereEqualTo("createdBy", user);
        query.whereEqualTo("position",position.trim());
        query.orderByDescending(params[1]); //typically order by date to get latest data string
        //add additional query paramters here if needed ( make sure to also include the params[index] for it ) ...
        try {
            List<ParseObject> results = query.find();
    ...
    public void pushParseListAltTap(ParseUser user, int numofitems, String ... params) //upload to Parse multiple rows - used for tapping test currently
    {
        ...
        while(i<numofitems)
        {
            try {
                ob = new ParseObject(params[0]);
                ob.put(params[1],params[i+2]); //2+i to get next paramater of different data,  params[2] and params[3] will be the data containers
                ob.put("username", ParseUser.getCurrentUser().getUsername());
                ob.put("createdBy", ParseUser.getCurrentUser());
                ob.put("position",params[i+10]); //get remaining parameters
                ob.put("numoftaps",params[i+18]);
                objectlist.add(i++, ob);
                //Toast.makeText(context, "Data uploaded to Parse",Toast.LENGTH_SHORT).show();
            } catch (Exception e) {
                Log.d("ParseError",e.getMessage());
